...
void MusicWidget::onPositionChanged(qint64 position)
{
    if(m_bSliderCanUpdate /* !m_playProgress->isSliderDown()*/)
        m_playProgress->setValue(position);   //设置进度条当前位置
...
void MusicWidget::initPlayer()
{
    ...
    connect(m_player, SIGNAL(durationChanged(qint64)),     //durationChanged会通知媒体时长，单位毫秒
            this, SLOT(onDurationChanged(qint64)));
    connect(m_player, SIGNAL(positionChanged(qint64)),    //positionChanged每当播放位置改变就发出，单位毫秒，实现进度条功能！
            this, SLOT(onPositionChanged(qint64)));

    m_playlist = new QMediaPlaylist(this);
    connect(m_playlist, SIGNAL(currentIndexChanged(int)),
            this, SLOT(onCurrentMediaIndexChanged(int)));
    m_player->setPlaylist(m_playlist);   //设置一个播放列表对象
