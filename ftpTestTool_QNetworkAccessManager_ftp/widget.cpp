...
void Widget::onGet()
{
    disableControls();  //开始时所有控件都不能使用了
    m_state->clear();   //清空状态文本框，主要靠这个QTextEdit显示状态！
    ...
    QUrl qurl(strURL);   //建立QUrl对象
    qurl.setUserName(m_user);
    qurl.setPassword(m_password);
    QNetworkRequest req(qurl);  //传入url信息给网络请求类
    ...
    m_reply = m_nam->get(req);   //返回QNetworkReply类来处理后续连接状态

    connect(m_reply, SIGNAL(error(QNetworkReply::NetworkError)),
            this, SLOT(onError(QNetworkReply::NetworkError)));
    connect(m_nam, SIGNAL(authenticationRequired(QNetworkReply*,QAuthenticator*)),
            this, SLOT(onAuthenticationRequired(QNetworkReply*,QAuthenticator*)));  //需要身份验证是发射此信号
