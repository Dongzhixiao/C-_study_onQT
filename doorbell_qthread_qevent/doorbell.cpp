...
void BellThread::run()
{
    ...
    QScopedPointer<Ringer> firer(new Ringer(this));  //把工作者Ringer交给只能指针QScopedPointer，就不用管其生死啦！
    exec();
}
...
void Ringer::onTimeout()
{
    ...
    QCoreApplication::postEvent(
                m_thread->m_receiver, ev);  //给主线程发送事件，跨线程发送最不要用sendEvent（）！！
    ...
    , m_movie(new QLabel)
{
    ...
    connect(t, SIGNAL(finished()), t, SLOT(deleteLater()));   //定义线程后先连接删除线程的信号和槽
    t->start();

    QVBoxLayout * layout = new QVBoxLayout;
    layout->addWidget(m_visitorLabel);

    QMovie * gif = new QMovie(":/eye.gif");   //使用gif资源的方法
...
bool Master::event(QEvent *e)
{
    ...
    return QWidget::event(e);   //最后未处理的事件要返回给上一级widget
}
