...
public:
    ...
    {
        QDir dir(m_path);
        //调用函数枚举目录下所有图片文件，返回的QFileInfoList对象中文件夹在前，按文件名排序
    ...
    , m_binFileIcon(":/images/binFile.png")
{
    ...
    setupGUI();
    setFocus();    
    m_worker = new WorkerThread();  //分配一个新的WorkerThread对象
    m_worker->start();
    m_worker->postRunnable(m_dirTraverse);  //把DirectoryTraverse对象提交给WorkerThread去执行
    m_pathLabel->setText(m_path);
}

OpenFileWidget::~OpenFileWidget()
{
    m_worker->quit();    //指示线程退出
    m_worker->wait(1000); //等待线程正真退出
    delete m_dirTraverse; //然后删除DirectoryTraverse对象
}
...
{//处理RunnableExcutedEvent事件，更新文件列表，根据文件类型给QListWidgetItem设置不同的图标
